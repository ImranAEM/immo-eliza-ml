...
def clean_data(datasheet):
    ...
    df_clean = df_clean.drop(columns=['Property ID', 'Price per m2'])
...
X_train, X_test, y_train, y_test, ct = preprocessing(cleaned_df)

save_preprocessed_data(X_train, X_test, y_train, y_test)

save_preprocessor(ct)
